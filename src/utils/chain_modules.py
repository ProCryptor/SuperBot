...
async def process_matcha_swap(route, chain_obj):
    # Аналогично Uniswap (копируй код из process_uniswap и замени Uniswap на MatchaSwap)
    # Для brevity — добавь сам, или используй заглушку:
    logger.success(f"[STUB] Matcha swap on {chain_obj.chain_name}")
    return True

async def process_bungee_swap(route, chain_obj):
    # Аналогично
    logger.success(f"[STUB] Bungee swap on {chain_obj.chain_name}")
    return True

async def process_relay_swap(route, chain_obj):
    # Аналогично
    logger.success(f"[STUB] Relay swap on {chain_obj.chain_name}")
    return True
...
MODULE_HANDLERS = {
    ...
    # ... твои другие задачи (bridge, vote и т.d.)
}
